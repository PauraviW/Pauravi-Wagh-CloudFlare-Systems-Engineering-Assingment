...
fn main() -> anyhow::Result<()> {
    let arguments = App::new("assignment")
        .version("1.0")
        .author("PauraviWagh")
        .arg(
            Arg::with_name("url")
    ...
    let path = url.path();
    match arguments.value_of("no_requests") {
        None => {
            let response_info = make_request(host, path)?;
            println!("{}", String::from_utf8_lossy(&response_info.response));
        }
        Some(i) => {
            let no_requests = i.parse::<usize>()?;

            let mut times = Vec::with_capacity(no_requests);
            let mut error_codes = Vec::with_capacity(no_requests);
            let mut sizes = Vec::with_capacity(no_requests);
            ...
            println!("Number of requests: {}.", no_requests);
            println!(
                "Fastest time: {:?}.",
                times
                    .iter()
            ...
            println!(
                "Success percentage: {}.",
                (no_requests - error_codes.len()) / no_requests
            );
...
fn make_request(host: &str, path: &str) -> anyhow::Result<ResponseData> {
